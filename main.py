...
def draw_score():
    ...
    score_text = score_font.render(f"Score: {shield.get_score()}", True, (0, 0, 0)) #suppose we have a var score instead of 0
            ...
            if event.type == TICKEVENT:
                ...
                for i in range(0, enemy_count):
                    ...
                    if (current_enemy.distance < 0.5):
                        flag = True
                    if (abs(current_enemy.get_distance() - shield.shield_distance) < 1 and abs(
                            current_enemy.get_angle_pos() - shield.get_angle_pos()) < 1):
